...
class SearchUsersResults extends Component {
  ...
  render() {
    ...
    const users = props.users ? Array.from(props.users).sort(itemsSort(sortBy)).map((user, index) => {
      const hasSubtitle = ((typeof user.subheader === 'string' && user.subheader.trim() !== '') ||
                           (user.city && user.state));

      const subtitle = hasSubtitle ? (
        <div>
          {(typeof user.subheader === 'string' && user.subheader.trim() !== '') && (
